...
const fileName = (nowMs: number): string => {
  ...
  return `dayporter-backup-${d.getFullYear()}${p(d.getMonth() + 1)}${p(d.getDate())}.json`;
};

/** Writes the backup and opens the share sheet. Throws on failure. */
export async function exportBackup(): Promise<void> {
  const now = Date.now();
  const { clients, sites, contracts, activities, bids } = getAllForBackup();
  const json = serializeBackup(clients, sites, contracts, activities, bids, now);
  ...
  await Sharing.shareAsync(uri, {
    ...
    dialogTitle: 'Export DayPorter backup',
  });
}
